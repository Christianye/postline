...
If bash_read rejects your command, the error string tells you exactly why — if it looks like a false positive, file an issue (see `feedback_tool_anomaly_reporting.md` principle: tool wrongness is C様's bug, not user's).
