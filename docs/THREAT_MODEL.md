...
| 4 | Sensitive data leaking in replies | Output redactor strips patterns: AWS access keys (`AKIA`/`ASIA`/…, case-insensitive) + secret keys (40-char near an aws/secret/key keyword), Anthropic keys (`sk-ant-…`), OpenAI keys (`sk-…`), Slack tokens (`xoxb-`/`xoxp-`/`xapp-`/…), GitHub tokens (classic `ghp_`/`gho_`/… + fine-grained `github_pat_`), Feishu secrets (32-char b64ish), Bearer tokens, PEM private-key blocks. Applied at the turn boundary, on the worker before it POSTs progress/result, and again bridge-side before the IM edit. | core |
| 5 | Tool abuse (rm -rf, force push) | Each tool declares `risk`. `dangerous` → inline-button approval (5-minute TTL) with a `/approve <id>` text fallback. `bash_read` is auto-approved but sandboxed (no command substitution, no shell-dispatch/`tee`, scrubbed env). Tool results logged for audit. | core/tool |
